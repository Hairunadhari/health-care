    ...
    <div class="relative flex-grow grid grid-cols-1 grid-rows-1">
                ...
                <div class="space-y-6">
                    <div
                        ...
                        <div class="flex items-center gap-4 w-full md:w-1/4 justify-center">
                            <div
                                class="w-12 h-12 bg-white rounded-xl flex items-center justify-center border-2 border-green-500">
                                <i class="fas fa-check-double text-green-500 text-2xl"></i>
                            </div>
                            <span class="text-2xl font-bold text-slate-800">08:00 Pagi</span>
                        </div>
                        <div
                            class="flex items-center gap-4 w-full md:w-1/4 justify-center cursor-pointer hover:opacity-70">
                        ...
                        <div class="w-full md:w-1/5 flex justify-end">
                            <button
                                class="bg-[#F97316] hover:bg-orange-600 text-black font-extrabold py-4 px-12 rounded-full shadow-lg uppercase tracking-wider transition-all active:translate-y-1">Minum</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div x-show="slide === 3" x-cloak x-transition:enter="transition transform duration-700 ease-in-out"
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div
                    class="bg-white/40 backdrop-blur-xl border border-white/30 rounded-[60px] p-10 md:p-16 shadow-2xl text-center md:text-left">
                    <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-12">
                        <img src="https://cdn-icons-png.flaticon.com/512/2693/2693510.png"
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="pt-10 flex flex-wrap justify-center md:justify-start gap-6">
                                <button
                                    class="bg-[#FF4D4D] text-white font-black py-4 px-10 rounded-full shadow-lg uppercase transition-all active:translate-y-1">Tunda</button>
                                <button
                                    class="bg-[#F97316] text-white font-black py-4 px-10 rounded-full shadow-lg uppercase transition-all active:translate-y-1">Minum</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="max-w-7xl mx-auto text-center">
                ...
                <div class="bg-white/30 backdrop-blur-md border border-white/40 rounded-[40px] p-12 shadow-xl">
                    ...
                    <div class="grid grid-cols-5 gap-4 md:gap-8 mb-12" x-data="{ selected: null }">
                        <button @click="selected = 1" :class="selected === 1 ? 'bg-purple-200' : 'bg-white/60'"
                            class="aspect-square rounded-3xl text-5xl hover:scale-105 transition shadow-sm border-2">😊</button>
                        <button @click="selected = 2" :class="selected === 2 ? 'bg-purple-200' : 'bg-white/60'"
                            class="aspect-square rounded-3xl text-5xl hover:scale-105 transition shadow-sm border-2">😐</button>
                        <button @click="selected = 3" :class="selected === 3 ? 'bg-purple-200' : 'bg-white/60'"
                            class="aspect-square rounded-3xl text-5xl hover:scale-105 transition shadow-sm border-2">😔</button>
                        <button @click="selected = 4" :class="selected === 4 ? 'bg-purple-200' : 'bg-white/60'"
                            class="aspect-square rounded-3xl text-5xl hover:scale-105 transition shadow-sm border-2">😫</button>
                        <button @click="selected = 5" :class="selected === 5 ? 'bg-purple-200' : 'bg-white/60'"
                            class="aspect-square rounded-3xl text-5xl hover:scale-105 transition shadow-sm border-2">😴</button>
                    </div>
                    <button
